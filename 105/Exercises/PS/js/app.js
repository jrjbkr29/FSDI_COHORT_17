...
function deletePet(petId){
    ...
    tr.remove();
    $('#delete-box').removeClass('unactive');
    setTimeout(function(){
        $('#alert-box').addClass('unactive');
    }, 3000);
}
...
function searchPet(){
    ...
    salon.pets.forEach(aPet=>{
        if(aPet.name.toLowerCase().includes(searchString || aPet.owner.toLowerCase().includes(searchString))){
...
window.onload=init;
    ...
    // for(var i=0;i<salon.pets.length;i++){
    //     var selected=salon.pets[i];
    //     if(searchString===selected.name.toLowerCase()|searchString===selected.gender.toLowerCase()||searchString===selected.breed.toLowerCase()||searchString===selected.service.toLowerCase()||searchString===selected.owner.toLowerCase()||searchString===selected.age.toString()||searchString===selected.phoneNum.toLowerCase()){
    //         $('#'+selected.id).addClass('selected');
    //     }else{
    //         $('#searchLabel').text("Not Found!");
    //         $('#searchLabel').css("color", "red");
    //         $('#searchLabel').delay(3000);
    //         $('#searchLabel').fadeOut();
